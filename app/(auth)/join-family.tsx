...
export default function JoinFamilyScreen() {
  ...
  async function handleJoin() {
    ...
    if (inviteCode.length !== 6) {
      Alert.alert("שגיאה", "נא להזין קוד של 8 ספרות");
      return;
    }
    const nameCheck = validateName(name);
    if (!nameCheck.valid) {
      Alert.alert("שגיאה", nameCheck.error);
      return;
    }
    const emailCheck = validateEmail(email);
    if (!emailCheck.valid) {
      Alert.alert("שגיאה", emailCheck.error);
      return;
    }
    const passCheck = validatePassword(password);
    if (!passCheck.valid) {
      Alert.alert("שגיאה", passCheck.error);
      return;
    }
      ...
      >
        ...
        >
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.label}>תפקיד במשפחה</Text>
            <View style={styles.roleRow}>
              ...
              >
                <Text
                  style={[
                    styles.roleBtnText,
                    role === "parent" && styles.roleBtnTextActive,
                  ]}
                >
                  הורה 👨‍👩‍👧
                </Text>
              ...
              >
                <Text
                  style={[
                    styles.roleBtnText,
                    role === "child" && styles.roleBtnTextActive,
                  ]}
                >
                  ילד/ה 🧒
                </Text>
